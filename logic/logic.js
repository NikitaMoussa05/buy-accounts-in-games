...
const sendProducts = (chat_id) => {
  ...
  const data = JSON.parse(
    fs.readFileSync("./db/products/products.json", "utf8")
  );
  data.map((i) => console.log(`login: ${i.login}\npassword: ${i.password}`));
  data.map((i) =>
    bot.sendMessage(chat_id, `login: ${i.login}\npassword: ${i.password}`)
  ); //когда надо отправить данные не ожидая от пользователя сообщения передаем chat_id
};
